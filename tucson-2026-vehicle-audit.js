/* NX4/NX4HEV 2026 Owners Manual vehicle-specific audit overlay.
 * Scope: Hyundai Tucson NX4 2026 and Tucson Hybrid NX4HEV 2026 only.
 * Sources:
 * - work/manual-text/NX4_2026_ko_KR.txt
 * - work/manual-text/NX4HEV_2026_ko_KR.txt
 * No TMED-2, Stay Mode, or V2L mapping is added because those terms are not present in the NX4HEV txt audit.
 */
window.applyTucsonManualAudit = function applyTucsonManualAudit() {
  ...
  const hevApply = {
    vehicleId: 'hy-tucson-hev',
    brand: '현대',
    model: '투싼 하이브리드',
    years: '2026',
    trim: 'NX4 HEV 해당 사양/트림',
    option: 'Owners Manual 수록 HEV 사양 기준'
  };

  if (!vehicleList.some(v => v.id === 'hy-tucson-hev')) {
    vehicleList.push({ id: 'hy-tucson-hev', brand: '현대', name: '투싼 하이브리드', group: 'SUV · 하이브리드', hybrid: true, performance: false });
  }
  ...
  const removeApply = id => {
    ...
    feature.applies = feature.applies.filter(apply => !['hy-tucson', 'hy-tucson-hev'].includes(apply.vehicleId));
  };

  const evidence = {
    ice: {
      manualFile: 'NX4_2026_ko_KR.txt',
      modelCode: 'NX4',
      modelName: '투싼',
      modelYear: '2026',
      basis: 'official_owner_manual',
      verifiedBy: 'vehicle-manual-audit',
      pageRange: 'NX4_2026_ko_KR.txt 본문 검색 확인'
    },
    hev: {
      manualFile: 'NX4HEV_2026_ko_KR.txt',
      modelCode: 'NX4HEV',
      modelName: '투싼 하이브리드',
      modelYear: '2026',
      basis: 'official_owner_manual',
      verifiedBy: 'vehicle-manual-audit',
      pageRange: 'NX4HEV_2026_ko_KR.txt 본문 검색 확인'
    }
  };
  ...
  const mark = (id, config = {}) => {
    ...
    feature.vehicleAudit = {
      ...(feature.vehicleAudit || {}),
      ...(config.ice !== false ? { hyTucson2026: { confirmed: true, manualFile: 'NX4_2026_ko_KR.txt', matchedKeywords: config.matchedKeywords || config.aliases || [feature.name], updatedAt: today } } : {}),
      ...(config.hev ? { hyTucsonHev2026: { confirmed: true, manualFile: 'NX4HEV_2026_ko_KR.txt', matchedKeywords: config.matchedKeywords || config.aliases || [feature.name], updatedAt: today } } : {})
    };
    return true;
  };
  ...
  adas.forEach(([id, slug, name, aliases, section, summary]) => {
    addManualFeature(id, {
      ...
      vehicleAudit: {
        hyTucson2026: { confirmed: true, manualFile: 'NX4_2026_ko_KR.txt', matchedKeywords: aliases, updatedAt: today },
        hyTucsonHev2026: { confirmed: true, manualFile: 'NX4HEV_2026_ko_KR.txt', matchedKeywords: aliases, updatedAt: today }
      }
    }, true);
  });
  ...
  const hevDetails = {
    'hybrid-smart-regen': {
      ...
      overview: '투싼 하이브리드 매뉴얼 기준 회생 제동은 차량 감속 및 제동 시 전기 모터의 이동 에너지를 전기 에너지로 변환해 배터리를 충전하는 기능입니다.',
    ...
    'hybrid-regen-level': {
      ...
      overview: '투싼 하이브리드 매뉴얼 기준 회생 제동은 감속 및 제동 때 배터리 충전을 위해 작동하며 주행 조건에 따라 제동감이 달라질 수 있습니다.',
    ...
    'hybrid-energy-flow': {
      ...
      overview: '투싼 하이브리드 매뉴얼 기준 하이브리드 에너지 흐름도는 주행 상태에 따른 시스템 동력 전달 상태와 엔진 상태를 클러스터에 표시합니다.',
    ...
    'hybrid-battery-status': {
      ...
      overview: '투싼 하이브리드 매뉴얼 기준 고전압 배터리 충전량 표시계는 시동 ON 상태에서 고전압 배터리 충전 상태를 표시합니다.',
    ...
    'hybrid-driving-info': {
      ...
      overview: '투싼 하이브리드 매뉴얼 기준 EV 모드 표시등은 모터를 이용해 주행하는 상태를 표시합니다.',
