/* NE1 2026 Owners Manual vehicle-specific audit overlay.
 * Scope: Hyundai IONIQ 5 NE1 2026 only. IONIQ 5 N (NE1N) is explicitly excluded.
 * Source:
 * - work/manual-text/NE1_2026_ko_KR.txt
 */
window.applyIoniq5ManualAudit = function applyIoniq5ManualAudit() {
  ...
  const evidence = {
    manualFile: 'NE1_2026_ko_KR.txt',
    modelCode: 'NE1',
    modelName: '아이오닉 5',
    modelYear: '2026',
    basis: 'official_owner_manual',
    verifiedBy: 'vehicle-manual-audit',
    pageRange: 'NE1_2026_ko_KR.txt 본문 검색 확인'
  };
  ...
  const removeIoniq5Apply = id => {
    const feature = findFeature(id);
    if (!feature || !Array.isArray(feature.applies)) return;
    feature.applies = feature.applies.filter(apply => apply.vehicleId !== 'hy-ioniq5');
  };
  const mark = (id, config = {}) => {
    const feature = findFeature(id);
    if (!feature) return false;
    upsertApply(feature, applyIoniq5);
    ...
    feature.vehicleAudit = {
      ...(feature.vehicleAudit || {}),
      hyIoniq5Ne12026: { confirmed: true, manualFile: 'NE1_2026_ko_KR.txt', matchedKeywords: config.matchedKeywords || config.aliases || [feature.name], updatedAt: today }
    };
    return true;
  };
  ...
  const addFeature = (id, feature) => {
    ...
    f.vehicleAudit = {
      ...(f.vehicleAudit || {}),
      hyIoniq5Ne12026: { confirmed: true, manualFile: 'NE1_2026_ko_KR.txt', matchedKeywords: feature.aliases || [feature.name], updatedAt: today }
    };
  };
  ...
  const evUpdates = {
    'plug-and-charge': {
      section: 'Plug n Charge',
      aliases: ['Plug n Charge', '플러그 앤 차지'],
      update: {
        overview: '아이오닉 5 매뉴얼 기준 Plug n Charge는 Electric Vehicle의 EV 설정에서 제공되는 전기차 특화 기능입니다.',
        settings: ['인포테인먼트 시스템 홈 화면에서 Electric Vehicle > EV 설정 > Plug n Charge를 선택합니다.'],
        steps: ['EV 설정에서 Plug n Charge 항목을 확인하고 안내에 따라 기능을 사용합니다.'],
    ...
    'ev-route-planner': {
      ...
      update: {
        overview: '아이오닉 5 매뉴얼 기준 충전소 검색 기능과 배터리 컨디셔닝 내비게이션 연동은 충전소 위치 확인 및 급속/초급속 충전소 목적지 설정과 연계됩니다.',
    ...
    'battery-conditioning': {
      ...
      update: {
        overview: '아이오닉 5 매뉴얼 기준 배터리 컨디셔닝은 고속 충전에 적합한 배터리 온도를 제공하기 위해 고전압 배터리 온도를 조절하는 기능입니다.',
        ...
        limitations: ['배터리 히터가 장착된 차량에서만 사용할 수 있습니다.', '배터리 충전량이 적거나 배터리 온도가 이미 주행 및 충전에 적합한 경우 작동하지 않을 수 있습니다.'],
    ...
    'scheduled-charging': {
      ...
      update: {
        overview: '아이오닉 5 매뉴얼 기준 예약 충전은 EV 충전 설정에서 완속 충전 조건과 출발 시간 연동을 설정해 충전 시간을 관리하는 기능입니다.',
    ...
    'charge-limit': {
      ...
      update: {
        overview: '아이오닉 5 매뉴얼 기준 충전 목표 배터리량 설정은 급속 충전 및 완속 충전 시 목표 배터리량을 설정하는 기능입니다.',
        ...
        disable: ['EV 충전 설정에서 목표 배터리량을 변경합니다.'],
    ...
    'v2l-parent': {
      ...
      update: {
        overview: '아이오닉 5 매뉴얼 기준 전기 사용(V2L)은 구동용 고전압 배터리의 전력을 외부 전기기기에 공급하는 기능입니다.',
    ...
    'utility-mode': {
      ...
      aliases: ['유틸리티 모드'],
      update: {
        overview: '아이오닉 5 매뉴얼 기준 유틸리티 모드는 EV 설정에서 제공되는 전기차 특화 기능입니다.',
    ...
    'ev-charge-status': {
      ...
      update: {
        overview: '아이오닉 5 매뉴얼 기준 충전 상태는 충전 도어 내부 또는 전면 범퍼 중앙의 충전 상태 표시등으로 확인합니다.',
        settings: ['차량 도어가 잠금 해제된 상태에서 충전 도어의 열림 표시부를 눌러 충전 도어를 엽니다.'],
        steps: ['충전 도어 내부의 충전 표시등을 확인합니다.', '전면 범퍼 중앙의 충전 표시등을 참고해 충전 상태를 확인합니다.', '0~100%까지 충전 상태가 4단계로 표시됩니다.'],
    ...
    'ev-energy-usage': {
      ...
      update: {
        overview: '아이오닉 5 매뉴얼 기준 에너지 정보 화면에서는 전력 소비량과 연비 이력을 확인할 수 있습니다.',
        settings: ['인포테인먼트 시스템 홈 화면에서 Electric Vehicle > 에너지 정보를 선택합니다.'],
        steps: ['에너지 정보 화면에서 전력 소비량을 선택합니다.', '구동계, 전자 장치, 공조 등 차량 내부 시스템별 현재 전력 소비량을 확인합니다.'],
    ...
    'ev-battery-health': {
      ...
      update: {
        overview: '아이오닉 5 매뉴얼 기준 고전압 배터리 상태와 주행 가능 거리는 충전량, 외기 온도, 주행 조건, 공조 사용에 따라 달라집니다.',
    ...
    'i-pedal': {
      ...
      update: {
        overview: '아이오닉 5 매뉴얼 기준 아이 페달(i-PEDAL)은 가속 페달만으로 차량을 가속하거나 감속 또는 정차할 수 있도록 돕는 기능입니다.',
        ...
        warnings: ['원 페달 드라이빙 기능으로 차량을 멈출 수 없을 수 있으므로 필요할 경우 직접 브레이크 페달을 밟아 감속하십시오.']
      }
    },
    'ev-smart-regeneration': {
      ...
      update: {
        overview: '아이오닉 5 매뉴얼 기준 스마트 회생 제동은 주행 상황에 따라 회생 제동을 보조하는 기능입니다.',
    ...
    'ev-regen-level': {
      ...
      update: {
        overview: '아이오닉 5 매뉴얼 기준 회생 제동은 스티어링 휠 레버로 제동량을 조절해 감속 에너지를 회수하는 기능입니다.',
        settings: ['스티어링 휠의 회생 제동 증가/감소 레버를 사용합니다.'],
        steps: ['회생 제동 증가 레버를 당겨 제동량을 높입니다.', '회생 제동 감소 레버를 당겨 제동량을 낮춥니다.', '차량 속도가 낮을 때 레버 조작 상태에 따라 정차까지 제동량이 증가할 수 있습니다.'],
        limitations: ['차량 또는 도로 상태에 따라 원 페달 드라이빙과 회생 제동 정지 기능이 제한될 수 있습니다.'],
  ...
  addFeature('ev-charging-current-setting', {
    ...
    settings: ['인포테인먼트 시스템 홈 화면에서 Electric Vehicle > EV 충전 설정 > 완속 충전 설정을 선택합니다.'],
    steps: ['완속 충전 설정에서 충전 전류를 설정합니다.', '휴대용 충전기 사용 시 제어 박스의 버튼을 2초 이상 길게 눌러 충전 전류를 설정합니다.'],
  ...
  addFeature('ev-menu-settings', {
    ...
    steps: ['Electric Vehicle 화면에서 에너지 정보를 선택해 전력 소비량과 연비 이력을 확인합니다.', 'EV 충전 설정에서 완속 충전 설정, 충전 목표 배터리량, V2L 설정을 조정합니다.', 'EV 설정에서 배터리 컨디셔닝, 유틸리티 모드, Plug n Charge를 확인합니다.'],
  ...
  addFeature('ev-charging-station-search', {
    ...
    steps: ['Electric Vehicle 메뉴에서 충전소 검색 기능을 실행합니다.', '주변 충전소 위치 또는 전기 충전소 목록을 확인합니다.', '필요한 충전소를 선택해 안내를 확인합니다.'],
  ...
  addFeature('v2l-discharge-limit', {
    ...
    preconditions: ['전기 사용(V2L) 기능을 사용할 수 있는 차량이어야 합니다.', '인포테인먼트 시스템의 Electric Vehicle 메뉴를 사용할 수 있어야 합니다.'],
    settings: ['인포테인먼트 시스템 홈 화면에서 Electric Vehicle > EV 충전 설정 > 전기 사용(V2L) 설정을 선택합니다.'],
    steps: ['전기 사용(V2L) 설정 화면에서 원하는 방전 제한량을 설정합니다.', '방전 제한량은 20% 이상 80% 이하로 설정할 수 있습니다.', '배터리 충전량이 설정된 방전 제한량에 도달하면 전기 사용(V2L) 기능이 자동으로 차단됩니다.'],
  ...
  addFeature('one-pedal-driving', {
    ...
    steps: ['회생 제동 증가 레버를 0.5초 이상 길게 당깁니다.', '당긴 상태를 유지하면 제동량이 증가하고 차량이 정차합니다.', '차량 속도가 3 km/h 이하일 때는 레버를 놓아도 차량이 정지할 때까지 제동량이 증가합니다.'],
