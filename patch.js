...
ensureDatabaseView = function () {
  ...
  picker.innerHTML = `
    ...
    <div class="picker-grid">
      ...
      <label>연식<select id="yearSelect"></select></label>
    </div>`;
  const searchBox = document.querySelector('.search-box');
  searchBox.insertAdjacentElement('afterend', picker);
};
...
renderDatabaseView = function () {
  ...
  brandSelect.innerHTML = ['전체', '현대', '기아', '제네시스'].map(brand => `<option value="${brand}" ${brand === state.brand ? 'selected' : ''}>${brand === '현대' ? '현대자동차' : brand}</option>`).join('');
  const brandVehicles = state.brand === '전체' ? vehicles : vehicles.filter(v => v.brand === state.brand);
  vehicleSelect.innerHTML = `<option value="">전체</option>${brandVehicles.map(v => `<option value="${v.id}" ${v.id === state.vehicleId ? 'selected' : ''}>${v.name}${v.future ? ' · 정보 제공 예정' : ''}</option>`).join('')}`;
  yearSelect.innerHTML = ['전체', '2027', '2026', '2025'].map(year => `<option value="${year}" ${year === state.year ? 'selected' : ''}>${year}</option>`).join('');
...
openModal = function (f, hasVehicleContext = false) {
  ...
  document.querySelector('#featureModalBody').innerHTML = `
    ...
    <p>${f.overview}</p>
    ${f.parent ? listHtml('기능 계층', [`${f.parent} › ${f.name}`]) : ''}
